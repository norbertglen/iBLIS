                            ...
                            <div class="panel-btn">
                                ...
                                @if(Auth::user()->can('verify_test_results') && Auth::user()->id != $test->tested_by)
                                    <a class="btn btn-sm btn-success"
                                       href="{{ URL::route('test.verify', array($test->id)) }}">
                        ...
                        @if($test->isCompleted() || $test->isVerified())
                            <div class="panel-btn">
                                @if(Auth::user()->can('view_reports'))
                                    ...
                                    <a class="btn btn-sm btn-default"
                                       href="{{ URL::to('patientreport/'.$test->visit->patient->id.'/'.$test->visit->id.'/'.$test->id ) }}">
                                        <span class="glyphicon glyphicon-eye-open"></span>
                    ...
                    <div class="col-md-6">
                        <div class="display-details">
                            ...
                            <p class="view-striped"><strong>{{trans('messages.registered-by')}}</strong>
                                {{(count($test->createdBy)>0)?$test->createdBy->name:"unknown" }}</p>
                            <p class="view"><strong>{{trans('messages.tested-by')}}</strong>
                                {{(count($test->testedBy)>0)?$test->testedBy->name:"unknown" }}</p>
                            @if($test->isVerified())
                            ...
                            <div class="panel-body">
                                <div class="container-fluid">
                                    <div class="row">
                                        ...
                                        <div class="col-md-9">
                                            {{$test->visit->patient->patient_number}}</div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-3">
                                            <p><strong>{{ Lang::choice('messages.name',1) }}</strong></p></div>
                                        <div class="col-md-9">
                                            {{$test->visit->patient->name}}</div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-3">
                                            <p><strong>{{trans("messages.age")}}</strong></p></div>
                                        <div class="col-md-9">
                                            {{$test->visit->patient->getAge()}}</div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-3">
                                ...
                                <div class="container-fluid">
                                    ...
                                    @if($test->specimen->isRejected())
                                        <div class="row">
                                            ...
                                            <div class="col-md-8">
                                                {{(count($test->specimen->rejectionReason)>0)?$test->specimen->rejectionReason->reason:trans('messages.pending') }}
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-4">
                                        ...
                                        <div class="row">
                                            <div class="col-md-4">
                                                <p><strong>{{trans('messages.date-rejected')}}</strong></p>
                                            </div>
                                            <div class="col-md-8">
	                                            <?php $timestamp=strtotime($test->specimen->time_rejected);?>
                                                {{date("Y-m-d",$timestamp)}}
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-4">
                                                <p><strong>{{trans('messages.time-rejected')}}</strong></p>
                                            </div>
                                            <div class="col-md-8">
                                                {{date("H:i",$timestamp)}}
                                            </div>
                                    ...
                                    @if($test->specimen->isReferred())
                                        ...
                                        <div class="row">
                                            <div class="col-md-4">
                                                <p>
                                                    <strong>@if($test->specimen->referral->status == Referral::REFERRED_IN)
